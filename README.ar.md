...
تم اختبارها والتحقّق منها مع **Ollama 0.32.x** (أحدث إصدار مستقرّ حالياً في أغسطس ٢٠٢٦). يستخدم LLMxRay نقاط Ollama التالية:
...
**متوافق مع:** Ollama 0.20 وما بعده (الإصدارات الأقدم تعمل للدردشة/التوليد لكن تفتقر إلى `think` ومخطّط `format` بصيغة JSON Schema). **مُوصى به:** Ollama 0.32+ — تصل القدرات وطول السياق مع قائمة النماذج، ويقبل `think` مستويات جهد متدرّجة، وتقبل التضمينات عرض `dimensions`.
