...
تحدّث مع أي نموذج Ollama وراقب وصول الرموز مع **تلوين الثقة** — كل رمز يُلوَّن وفق سرعة توليده. يدعم Markdown، والمحادثات متعددة الأدوار، ومرفقات الملفات، ونماذج الرؤية، والأوامر بالشرطة المائلة. وللنماذج الاستدلالية، اضبط ميزانية التفكير لكل محادثة — إيقاف، أو ترك الاختيار للنموذج، أو جهد صريح منخفض / متوسط / مرتفع / أقصى.
...
ضمّن النصوص، وصوّر المتجهات، وقِس تشابه جيب التمام. اطلب متجه إخراج أضيق لترى كم يكلّف اقتطاع ماتريوشكا من التشابه. ابنِ قاعدة معرفة محلية من ملفات PDF و DOCX و CSV — مقطَّعة، مضمَّنة، وقابلة للبحث. كل ذلك مخزَّن في IndexedDB. بلا تكلفة.
...
| `/api/tags` | قائمة النماذج والقدرات وطول السياق وعرض التضمين |
| `/api/show` | المعاملات والقالب والترخيص وبيانات البنية |
...
**متوافق مع:** Ollama 0.20 وما بعده (الإصدارات الأقدم تعمل للدردشة/التوليد لكن تفتقر إلى `think` ومخطّط `format` بصيغة JSON Schema). **مُوصى به:** Ollama 0.33.3+ — يُبلَّغ عن إعادة استخدام ذاكرة المُوجّه المؤقتة (`prompt_eval_cached_count`، و`usage.prompt_tokens_details.cached_tokens` على النقطة المتوافقة مع OpenAI)، فيُقاس معدّل التعبئة المسبقة على الرموز المُقيَّمة فعلياً. منذ 0.32: تصل القدرات وطول السياق مع قائمة النماذج، ويقبل `think` مستويات جهد متدرّجة، وتقبل التضمينات عرض `dimensions`.
