...
function getWindowsInfo() {
  const osInfo = psJson('Get-CimInstance Win32_OperatingSystem | Select-Object Caption,Version,BuildNumber')
  const csInfo = psJson('Get-CimInstance Win32_ComputerSystem | Select-Object Name,Manufacturer,Model,SystemFamily')
  const cpuInfo = psJson('Get-CimInstance Win32_Processor | Select-Object Name,NumberOfCores,NumberOfLogicalProcessors,MaxClockSpeed')
  const gpuInfoRaw = psJson('Get-CimInstance Win32_VideoController | Select-Object Name,AdapterRAM,DriverVersion')
  const diskInfoRaw = psJson('Get-CimInstance Win32_LogicalDisk -Filter \\"DriveType=3\\" | Select-Object DeviceID,Size,FreeSpace')
  ...
  return {
    ...
    gpu: gpuList.length > 0
      ? gpuList.map((g) => ({ name: g.Name ?? 'Unknown', adapterRamBytes: g.AdapterRAM ?? 0, driverVersion: g.DriverVersion ?? '' }))
      : [{ name: 'Not detected', adapterRamBytes: 0, driverVersion: '' }],
    storage: diskList.map((d) => ({ drive: d.DeviceID ?? '', totalBytes: d.Size ?? 0, freeBytes: d.FreeSpace ?? 0 })),
  }
}
...
function getLinuxInfo() {
  ...
  const gpus = gpuLines.map((line) => ({ name: line.replace(/^[^:]+:\s*/, ''), adapterRamBytes: 0, driverVersion: '' }))
  ...
  const storage = dfLines.filter(Boolean).map((line) => {
    ...
    return { drive: parts[0] ?? '/', totalBytes: parseInt(parts[1] ?? '0', 10) || 0, freeBytes: parseInt(parts[2] ?? '0', 10) || 0 }
  })
  const prettyName = exec("grep PRETTY_NAME /etc/os-release | cut -d= -f2 | tr -d '\"'")

  return {
    os: { name: prettyName || `Linux ${os.release()}`, version: os.release(), build: '', arch: os.arch() },
    device: {
      name: os.hostname(),
    ...
    cpu: { name: cpuModel || (os.cpus()[0]?.model ?? 'Unknown'), cores: cpuCores, logicalProcessors: os.cpus().length, maxClockMhz: os.cpus()[0]?.speed ?? 0 },
...
function getMacInfo() {
  ...
  return {
    os: { name: `macOS ${swVers}`, version: swVers, build: exec('sw_vers -buildVersion'), arch: os.arch() },
    device: { name: os.hostname(), manufacturer: 'Apple', model: exec('sysctl -n hw.model'), family: '' },
    cpu: { name: cpuBrand || (os.cpus()[0]?.model ?? 'Unknown'), cores: cpuCores, logicalProcessors: logicalCores, maxClockMhz: os.cpus()[0]?.speed ?? 0 },
    memory: { totalBytes: os.totalmem(), freeBytes: os.freemem() },
    gpu: [{ name: gpuName || 'Integrated', adapterRamBytes: gpuVram ? parseInt(gpuVram, 10) * 1024 * 1024 : 0, driverVersion: '' }],
    storage: [{ drive: '/', totalBytes: totalBlocks * 512, freeBytes: availBlocks * 512 }],
  }
}
